...
def main():
    print("Training V7 Calibration (Context-Aware CQR)...")
    ...
    if 'hybrid_p50' not in df.columns:
        if 'meta_p50' in df.columns:
            df['hybrid_p50'] = df['meta_p50']
        elif 'prod_p50' in df.columns:
             df['hybrid_p50'] = df['prod_p50'] # Use production p50 as base if meta not explicit
        
    cols = ['y_true', 'analog_p50', 'timesfm_p50', 'chronos_p50', 'horizon_step', 'hybrid_p50', 'ctx_mean', 'ctx_std', 'ctx_slope']
    ...
    # Features for Uncertainty Model (V7 adds Context Features)
    ...
    df['v7_p10'] = df['raw_p10'] - q_adj
    df['v7_p90'] = df['raw_p90'] + q_adj
    
    # Clip
    df['v7_p10'] = df['v7_p10'].clip(lower=0)
    
    # 5. Evaluate V7 vs V5 (Baseline)
    ...
    if 'prod_p10' in df.columns:
        m_v5 = get_metrics("V5/V6 (Baseline)", df['prod_p10'], df['prod_p90'], df['y_true'], df['hybrid_p50'])
        print("\nComparison Results:")
    ...
    # V7 (Dynamic)
    m_v7 = get_metrics("V7 (Context-Aware CQR)", df['v7_p10'], df['v7_p90'], df['y_true'], df['hybrid_p50'])
    print(f"{m_v7[0]:<25} {m_v7[1]:<10.1f} ${m_v7[2]:<14,.0f} {m_v7[3]:<10.0f} {m_v7[4]:,.0f}")
    
    # Train Final Models on Full Data
    print("\nTraining Final V7 Models on full dataset...")
    model_lo = xgb.XGBRegressor(objective='reg:quantileerror', quantile_alpha=0.1, n_estimators=300, max_depth=5)
    model_hi = xgb.XGBRegressor(objective='reg:quantileerror', quantile_alpha=0.9, n_estimators=300, max_depth=5)
    
    model_lo.fit(X, y_res)
    model_hi.fit(X, y_res)
    
    # Save models
    output_dir = Path("models/calibration_v7")
    ...
    print(f"V7 Artifacts saved to {output_dir}")
