...
* [AlignTune — Efficient LLM Alignment: SimPO-primed DPO (QLoRA-8B); seeded ablations; symmetric judge win-rate; safety probes; FastAPI/Gradio; FAISS-RAG](./AlignTune)
  ...
  - Deterministic data packs, streaming ingestion, near‑dup removal, toxicity skim, per‑source caps, record‑level provenance; length‑bucketed ROUGE‑L, unbiased LLM‑judge win‑rate (greedy, symmetric, anti‑position, rubric + JSON), toxicity per 1k tokens; FastAPI inference server + Gradio demo with versioned LoRA adapters/tokenizer snapshots for drop‑in deployment.
